...
def process_emissions_for_file(tracker, script_path, emissions_csv, customer_name, file_type, result_dir, test_command):
    emissions_data = None
    duration = 0
    test_output = 'Unknown'
...
def process_folder(BASE_DIR, EMISSIONS_DATA_CSV, RESULT_DIR, suffix):
    ...
    if not os.path.exists(EMISSIONS_DATA_CSV):
        with open(EMISSIONS_DATA_CSV, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([
                "Customer Name", "Application name", "File Type", "Timestamp", "Emissions (gCO2eq)",
